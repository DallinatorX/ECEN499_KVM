...
void setup()
{
  ...
  // Begin I2C Bus
  Serial1.begin(9600);             // join i2c bus with address #4
///  Serial1.onReceive(receiveEvent); // register event
}
...
void serialEvent1()
{
  // while(Serial1.available() > 0) // loop through all avaliable
  {
    // Get Object Type
    action = 0;
    type_in = Serial1.read(); // receive byte
    Serial.print(type_in);
    ...
    // Mouse Move Events
        ...
        mouse_x = Serial1.read();
        mouse_y = Serial1.read();
        action = 3;
    }
    // Mouse Click Events
